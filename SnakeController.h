...
public:
	...
	const ModelIndex& GetModelIndex(short const& pos_x, const short&) override { 
		auto smth_seg=body_snake.begin();
		for (short i = 0; i != pos_x; ++i,++smth_seg);
		return smth_seg->data;
	};
	void SetModelIndex(const ModelIndex& smth_index, const short& pos_x, const short&) override {
		auto smth_seg = body_snake.begin();
		for (short i = 0; i != pos_x; ++i, ++smth_seg);
