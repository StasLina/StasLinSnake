...
void MapController::Move() {
    ...
    while (smth_snake != smth_snake_end) {
        ...
        if (GlobalVars::isEated) {
            ...
            cur_map_model->SetModelIndex(' ', segment.cur_pos.x, segment.cur_pos.y);
            addEat();
        }
        ++smth_snake;
    }
...
void MapController::Draw() {
    ...
    for (int j = 0; j != my_indent.top; ++j) {
        ...
        res_map_str += '$';
        for (short cur_col = 0; cur_col != GlobalVars::x_size; ++cur_col) {
        ...
        std::string str_score = "\speed: " + std::to_string(GlobalVars::speed_time);
    ...
    for (short cur_row = 0; cur_row != GlobalVars::y_size; ++cur_row) {
        ...
        res_map_str += '$';
        for (short cur_col = 0; cur_col != GlobalVars::x_size; ++cur_col) {
            res_map_str += this->cur_map_model->GetModelIndex(cur_col, cur_row).data;
        }
        res_map_str += '$';
        for (int i = 1; i != my_indent.right; i++) {
            res_map_str += fill_char;
        ...
        if (score_snake != score_snake_end) {
            char ch=(*score_snake)->GetModel()->GetModelSegment(0).data.data;
            std::string str_score = "\tscore ";
            str_score += ch; //+ ((*score_snake)->GetModel()->GetModelSegment(0).data.data)
            str_score += " : ";
    ...
    if (GlobalVars::isGameOver) {
        ...
        while (smth_snake != smth_snake_end) {
            if ((*smth_snake)->isLife()) {
                std::cout << "«мейка " << (*smth_snake)->GetModel()->GetModelSegment(0).data << " победила" << std::endl;
            }
            ++smth_snake;
        }
    }
